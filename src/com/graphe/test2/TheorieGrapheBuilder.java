...
public class TheorieGrapheBuilder {
	...
	public void setVilleClic(Ville ville) {
		...
		if (!ville.bouton.isSelected()) {
			...
			if (nbBoutonClic == 1) {
				...
				villeDepart.poids = 0;
				nbBoutonClic = 1;
			/* si déjà deux villes ont été sélectionnées (par exemple après un calcul)
			 * on remet les paramètres en position initial,
			 * on selectionne la ville cliquée comme nouvelle ville de départ
			 */
			} else {
	...
	private void initialize() {
		...
		carte.setIcon(new ImageIcon("C:/Users/nico/eclipse-workspace/TheorieGraphe/carte.jpeg"));
